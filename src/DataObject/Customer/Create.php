...
class Create extends AbstractDataObject
{
    ...
    protected static function describeStructure(): Structure
    {
        return Structure::build()
            ->addProperty(
                Property::build('username')
                    ->type(Structure\DataType::STRING)
                    ->rules(
                        RuleSet::build()
                            ->addRule(Rule::notEmpty())
                            ->addRule(Rule::lengthBetween(8, 64))
                            ->addRule(
                                Rule::regex('/^[a-zA-Z\d_\-.@]+?$/')->error(
                                    'The username has invalid format, it can contain ' .
            ...
            ->addProperty(
                Property::build('password')
                    ->type(Structure\DataType::STRING)
                    ->rules(
                        RuleSet::build()
                            ->addRule(Rule::notEmpty())
                            ->addRule(Rule::lengthBetween(8, 255))
                    )
            )
            ->addProperty(
